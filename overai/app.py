...
class Api:
    ...
    def _get_hwnd(self):
        if self._hwnd is None and self.window:
            # pywebview exposes the native window handle on Windows
            try:
...
def _setup_window(window, api):
    """Called once the window is shown; applies Win32 tweaks and injects toolbar."""
    api.set_window(window)
    hwnd = api._get_hwnd()
    if hwnd:
        # Hide from screen captures / screen sharing
        SetWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE)
    # Inject the toolbar into the initial page
...
def create_app(tray_setup_fn=None, hotkey_setup_fn=None):
    """
